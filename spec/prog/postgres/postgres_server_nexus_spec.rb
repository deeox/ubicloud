...
CMD
  ...
  describe "network metering" do
    ...
    describe "#install_network_metering" do
      ...
      it "no-ops on metal providers even when the flag is on" do
      ...
      it "no-ops when the rhizome binaries are absent on the VM" do
        allow(Config).to receive(:pg_network_metering_enabled).and_return(true)
        expect(sshable).to receive(:_cmd).with(/test -x postgres\/bin\/apply-metering-config/).and_return("NO\n")
        nx.install_network_metering("aws")
      end

      it "writes config, applies nft, and enables systemd units when binaries exist" do
        allow(Config).to receive_messages(pg_network_metering_enabled: true, control_plane_outbound_cidrs: ["0.0.0.0/0", "::/0"])
        expect(sshable).to receive(:_cmd).with(/test -x postgres\/bin\/apply-metering-config/).and_return("YES\n")
        ...
        expect(sshable).to receive(:_cmd).with("sudo systemctl enable --now pg-metering-export.timer")
        nx.install_network_metering("aws")
      end
    end
  end
