...
RSpec.describe NetworkMetering::Provider::Gcp do
  ...
  describe "#fetch_ranges" do
    it "fetches cloud.json and goog.json" do
      expect(Excon).to receive(:get).with(described_class::CLOUD_RANGES_URL, expects: 200).and_return(instance_double(Excon::Response, body: '{"prefixes":[]}'))
      expect(Excon).to receive(:get).with(described_class::GOOG_RANGES_URL, expects: 200).and_return(instance_double(Excon::Response, body: '{"prefixes":[]}'))
